...
(function (global, undefined) {
    ...
    if (!string.prototype.format) {
        string.prototype.format = function () {
            let i, result = this, isNumber = function (n) {
                return !isNaN(parseFloat(n)) && isFinite(n);
            }, Formatter = (function () {
                let Constr = function (identifier) {
                    let array = function (len) { return new Array(len); };
                    switch (true) {
                        case /^#\{(\w+)\}*$/.test(identifier):
                            this.formatter = function (line, param) {
                                let re = new RegExp('#{' + RegExp.$1 + '}', 'g');
                        ...
                        case /^(o)$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                return line.replace("%" + identifier, parseInt(param).toString(8));
                            };
                            break;
                        // Binary
                        case /^(b)$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                return line.replace("%" + identifier, parseInt(param).toString(2));
                            };
                            break;
                        // Hex
                        case /^([xX])$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                let hex = parseInt(param).toString(16);
                                if (identifier === 'X') {
                                    hex = hex.toUpperCase();
                                }
                        ...
                        case /^(-?)(\d*).?(\d?)(e)$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                let lpad = RegExp.$1 === '-', width = RegExp.$2, decimal = RegExp.$3 !== '' ? RegExp.$3 : undefined, val = param.toExponential(decimal), mantissa, exponent, padLength;
                                if (width !== '') {
                        ...
                        case /^(-?)(\d*).?(\d?)(f)$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                let lpad = RegExp.$1 === '-', width = RegExp.$2, decimal = RegExp.$3, DOT_LENGTH = '.'.length, integralPart = param > 0 ? Math.floor(param) : Math.ceil(param), val = parseFloat(param).toFixed(decimal !== '' ? decimal : 6), numberPartWidth, spaceWidth;
                                if (width !== '') {
                                    if (decimal !== '') {
                                        numberPartWidth =
                                        ...
                                        val = parseFloat(param).toFixed(width - (integralPart.toString().length + DOT_LENGTH));
                                    }
                                }
                                return line.replace("%" + identifier, val);
                            };
                        ...
                        case /^([0\-]?)(\d+)d$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                let len = RegExp.$2 - param.toString().length, replaceString = '', result;
                                if (len < 0) {
                                    len = 0;
                                }
                                switch (RegExp.$1) {
                        ...
                        case /^(-?)(\d+)s$/.test(identifier):
                            this.formatter = function (line, param) {
                                ...
                                switch (RegExp.$1) {
                                    case "": // rpad
                                        //replaceString = (array(len + 1).join(" ") + param).slice(-RegExp.$2);
                                        replaceString = (array(len + 1).join(" ") + param).slice(0, RegExp.$2);
                                        break;
                                    case "-": // lpad
                                        replaceString = (param + array(len + 1).join(" ")).slice(0, RegExp.$2);
                                        break;
                                    default:
                                    // TODO throw ?
                                }
                        ...
                        case /^(-?\d*)\.(\d+)s$/.test(identifier): //TODO fixx removing chars at end
                            this.formatter = function (line, param) {
                                let replaceString = '', max, spacelen;
                                // %.4s
                                if (RegExp.$1 === '') {
